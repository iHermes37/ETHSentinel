...
func (M *Mnemonic) NewMnemonic(bits int) (string, error) {
...
func (M *Mnemonic) NewMnemonicFromEntropy(entropy []byte) (string, error) {
	return bip39.NewMnemonic(entropy)
}

// NewEntropy returns a randomly generated entropy.
func (M *Mnemonic) NewEntropy(bits int) ([]byte, error) {
...
func (s *Seed) NewSeed() ([]byte, error) {
	b := make([]byte, 64)
	_, err := rand.Read(b)
	return b, err
}
...
func (s *Seed) NewSeedFromMnemonic(mnemonic string, passOpt ...string) ([]byte, error) {
	if mnemonic == "" {
...
func (dp *DerivationPath) ParseDerivationPath(path string) (accounts.DerivationPath, error) {
...
func (dp *DerivationPath) MustParseDerivationPath(path string) accounts.DerivationPath {
...
func (w *Wallet) NewWallet(seed []byte) (*Wallet, error) {
...
func (w *Wallet) NewFromMnemonic(mnemonic string, passOpt ...string) (*Wallet, error) {
	if mnemonic == "" {
		return nil, errors.New("mnemonic is required")
	}
	...
	seed, err := w.seed.NewSeedFromMnemonic(mnemonic, passOpt...)
	if err != nil {
		return nil, err
	}

	wallet, err := w.NewWallet(seed)
	if err != nil {
		return nil, err
	}
...
func (w *Wallet) NewFromSeed(seed []byte) (*Wallet, error) {
	if len(seed) == 0 {
		return nil, errors.New("seed is required")
	}

	return w.NewWallet(seed)
}
