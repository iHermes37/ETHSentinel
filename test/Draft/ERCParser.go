...
func DetectERCStandard(tx *types.Transaction) (types.ERCStandard, error) {
	data := tx.Data()

	// ERC20
	erc20ABI, _ := abi.JSON(strings.NewReader(Erc.erc20ABIJSon))
	if _, err := erc20ABI.MethodById(data[:4]); err == nil {
		erc := types.ERCStandard{}
		erc.Ercname = types.ContractType("ERC20")
	...
	if _, err := erc721ABI.MethodById(data[:4]); err == nil {
		erc := types.ERCStandard{}
		erc.Ercname = types.ContractType("ERC721")                    // ✅ 修改
		erc.Opmethod, erc.Params = Erc.ParseERC721Tx(data, erc721ABI) // ✅ 修改 ABI
		return erc, nil
	}

	return types.ERCStandard{}, fmt.Errorf("未识别 ERC 标准")
}
