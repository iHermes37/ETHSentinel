...
func ParseDexInput(tx *types.Transaction, dexname string) types.DeFiTxDetail {
	cli := init.InfuraConn(rpcURL, proxyStr)
	if dexname == "" {
		return ParseUniswapV2(*tx, cli)
	}

	return types.DeFiTxDetail{}
}
...
func ParseUniswapV2(tx types.Transaction, cli *ethclient.Client) types.DeFiTxDetail {
	var d = types.DeFiTxDetail{}
	data := tx.Data()
	if len(data) < 4 {
		log.Fatalf("	1")
	}
	...
	if method.Name == "swapExactTokensForTokens" {
		...
		d.Direction = types.TxDirectionOut // 假设是用户主动 swap
	}
	return d
}
