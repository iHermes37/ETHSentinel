...
func (c *CrossDEXStrategy) Run(stop <-chan struct{}) {
	// 套利机会通道
	arbCh := make(chan types.CrossPairData, 100)
	// ----------------- 套利执行 worker -----------------
	go func() {
...
// ========================================================================
// 执行套利合约调用
func (c *CrossDEXStrategy) ExecuteArbitrage(pair types.CrossPairData) {
	// 1. 初始化客户端
...
// 防止mev
func (c *CrossDEXStrategy) SubmitFlashbotsBundle(pair types.CrossPairData) error {
	// 1. 构建交易数据，调用套利合约
...
// 决定套利方向和基准代币
func (c *CrossDEXStrategy) DecidePullToken(p *types.CrossPairData) {
	...
	default:
		p.Direction = 0
		p.PullToken = types.Token{}
	}
}

// 计算套利利润
func (c *CrossDEXStrategy) CalBN(p *types.CrossPairData) types.ArbitrageOpportunity {
	...
	if p.PullToken == p.Pair_DexA.Token0 {
		...
		return types.ArbitrageOpportunity{Profit: big.NewFloat(0)}
	}
...
// 验证并记录
func (c *CrossDEXStrategy) ValidateAndRecord(p *types.CrossPairData) {
	db.AddToMysql(p)
}
