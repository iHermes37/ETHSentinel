...
type ContractQueryParams struct {
	...
	ContractType *types.ContractType `json:"contractType"` // 对应 JSON 驼峰命名
...
type ContractQueryResponse struct {
	...
	ContractType types.ContractType `json:"contractType"`
	ContractAge  time.Duration      `json:"contractAge"`
	DeployTime   time.Time          `json:"deployTime"`
	TxHash       common.Hash        `json:"txHash"`
}
...
type AddKnownContractsParam struct {
	Category    types.ContractType
	Protocol    types.ConstractProto
	RouterAddr  common.Address
	FactoryAddr common.Address
	Method      types.ConstractMethod
}
