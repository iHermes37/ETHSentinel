...
type ArbitrageOpportunityResponse struct {
	PairDexA  types.Pair `json:"pairDexA"`  // DEX A 的交易对信息
	PairDexB  types.Pair `json:"pairDexB"`  // DEX B 的交易对信息
	Direction string     `json:"direction"` // 套利方向 "A->B" or "B->A"

	BorrowToken types.Token `json:"borrowToken"` // 借入代币
...
type DexLiquidityResponse struct {
	...
	Token0      types.Token            `json:"token0"`
	Token1      types.Token            `json:"token1"`
...
type DexLiquidityChangeResponse struct {
	...
	Token0      types.Token                    `json:"token0"`
	Token1      types.Token                    `json:"token1"`
	Changes     []types.DexPairLiquidityChange `json:"changes"` // 变化量事件序列
}
