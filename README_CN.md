...
```go
import "github.com/ETHSentinel/sentinel"
...
```go
import "github.com/ETHSentinel/sentinel/internal/service/whale"
...
```go
mempoolCh, _ := client.Mempool().Subscribe(ctx,
    sentinel.FilterByMinValueETH(1.0),        // >1 ETH
    sentinel.FilterByMethod("0x38ed1739"),     // UniswapV2 swap
)
for ptx := range mempoolCh {
    fmt.Println("pending:", ptx.Tx.Hash().Hex())
}
```
...
```go
client, _ := sentinel.New(
    sentinel.WithMnemonic("word1 word2 ... word12"),
)
wallet, _ := client.Wallet()
addr, _ := wallet.Address(0)
hash, _ := wallet.Send(ctx, 0, toAddr, big.NewInt(1e18), nil)
```

**添加新链（如 Optimism）只需一行：**

```go
// 在 internal/chain/chains.go 添加
func Optimism() Chain {
    return NewBaseChain(chain.ChainOptimism, "optimism", "ETH",
        "https://mainnet.optimism.io", "wss://ws-mainnet.optimism.io")
}
// 在 registry.go 的 init() 里加一行
Register(Optimism())
```